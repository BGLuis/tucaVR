    ...
    if (state.frameCount % 6 == 0) {
        ...
        // condicionar a compilacao nativa por build type.
        {
            ...
            snprintf(hud, sizeof(hud),
                "VULKAN | %s | stereoLayout=%d polar180=%d swap=%d | video=%s vidGap=%.0fms vidFps=%.0f decFps=%.0f outFps=%.0f drop=%.0f jitter=%.0fms | net=%.1fMB/s q=%u | %.0ffps %.1fms stutter=%d freeze=%d",
                ...
                state.netMBs, state.videoQueueDepth,
