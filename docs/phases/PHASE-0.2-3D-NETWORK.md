...
- [x] **T1.1** — Implementar shader de separação SBS (C++/GLSL):
...
- [x] **T1.2** — Implementar shader de separação OU (C++/GLSL):
    ```glsl
    void main() {
    ...
    ```
- [x] **T1.3** — Suportar variantes Half e Full:
...
- [x] **T1.4** — Implementar seleção de modo no UI:
...
- [x] **T1.5** — Swap eyes: Opção para inverter olho esquerdo/direito (alguns conteúdos estão invertidos)
...
- [x] **T2.1** — Gerar geometria de **esfera UV** (C++):
...
- [x] **T2.2** — Implementar shader para projeção **equirectangular**:
    - Textura 2:1 → esfera completa
    - Sampler com wrap mode `REPEAT` em U, `CLAMP_TO_EDGE` em V
- [x] **T2.3** — Implementar **180° VR**:
...
- [x] **T2.4** — Implementar **360° estereoscópico**:
    ...
    ```
- [x] **T2.5** — Implementar **180° estereoscópico**:
    - Similar a 360° stereo, mas geometria limitada a 180°
- [x] **T2.6** — Posicionar o **usuário no centro** da esfera:
...
- [x] **T2.7** — Desabilitar tela virtual e ambientes durante reprodução 360°/180°:
...
- [x] **T3.2** — Detecção por **filename pattern** (fallback):
...
- [x] **T3.3** — Detecção por **heurística de resolução** (último recurso):
...
- [x] **T6.1** — Implementar cliente **FTP** no Rust:
...
- [x] **T6.2** — Implementar cliente **SFTP** no Rust:
    - Crate: `russh` + `russh-sftp` (puro Rust, sem `libssh2`/OpenSSL nativos — evita o cross-compile C que o `[!WARNING]` abaixo descreve para a alternativa `ssh2`)
    - Autenticação: password, key-based (`SftpTarget::private_key`, PEM em memoria)
    - Operações: opendir, readdir (com stat embutido na resposta), open, read (posicional, sem seek — ver T6.3)
...
_Fase 0.2 — Estimativa: 8-14 semanas para desenvolvedor solo experiente (após v0.1 estável)_
_Dependência forte: v0.1 DEVE estar completa e estável antes de iniciar v0.2_
